...
На втором этапе группового проекта нужно сделать алгоритм , решения задачи в соответствии с нашей темой. Итак прежде чем описывать непосредственные этапы алгоритма скажем о том для чего он нужен. Алгоритм — это упорядоченный набор действий, который необходимо выполнить для решения поставленной задачи. Алгоритмы нужны для: Получения результата более эффективным и быстрым путем и уменьшения количества ошибок, которые возникают при решении задач вручную.
